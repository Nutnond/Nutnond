<!-- 🔥 ULTRA PRO GITHUB PROFILE README -->

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?size=28&duration=3000&color=00F7FF&center=true&vCenter=true&width=600&lines=Hi+I'm+Nut+👋;Software+Developer;HealthTech+Builder;Mentor+%26+Innovator" />
</p>

<h1 align="center">🚀 Welcome to My World</h1>

<p align="center">
  💻 Developer @ Naresuan University Hospital <br/>
  🎓 SIT, KMUTT Graduate <br/>
  👨‍🏫 National-level Project Mentor <br/>
  ⚡ Building Tech That Actually Matters
</p>

---

## 🧠 About Me

```diff
+ Obsessed with performance, system design, and clean, production-grade architecture
+ Mentored teams to compete and win at national-level competitions
+ Ship fast. Iterate faster. Deliver impact. 🚀
```

---

## 🛠 Tech Arsenal

<p align="center">
  <img src="https://skillicons.dev/icons?i=js,ts,python,vue,nuxtjs,nodejs,express,mysql,postgres,docker,linux,git" />
</p>

---

---

## 📫 Connect

<p align="center">
  <a href="mailto:your_email@gmail.com">Email</a> •
  <a href="https://www.linkedin.com/in/YOUR_LINKEDIN">LinkedIn</a>
</p>

---

<p align="center">
  ⚡ <i>Code. Impact. Repeat.</i>
</p>
